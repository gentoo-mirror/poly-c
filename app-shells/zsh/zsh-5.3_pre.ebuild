...
inherit flag-o-matic prefix poly-c_ebuilds
...
if [[ ${MY_PV} == 9999* ]] ; then
	...
	SRC_URI="http://www.zsh.org/pub/${MY_P}.tar.xz
		doc? ( http://www.zsh.org/pub/${MY_P}-doc.tar.xz )"
fi
...
src_prepare() {
	if [[ ${MY_PV} != 9999* ]]; then
		...
		eapply "${FILESDIR}"/${PN}-init.d-gentoo-r2.diff
	fi
	...
	if [[ ${MY_PV} == 9999* ]] ; then
		sed -i "/^VERSION=/s/=.*/=${MY_PV}/" Config/version.mk || die
		eautoreconf
	fi
}
