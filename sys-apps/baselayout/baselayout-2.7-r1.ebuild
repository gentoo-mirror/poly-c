...
# $Id: 3d5fac97ac66ae260b8156866edde514f08872da $
	...
	if ! use split-usr ; then
		for libdir in ${libdirs}; do
			if [[ ! -e "${EROOT}${libdir}" ]]; then
				ln -s usr/"${libdir}" "${EROOT}${libdir}" || die " Unable to make ${EROOT}${libdir} symlink"
			fi
		done
	fi
}
...
src_prepare() {
	...
	eapply "${FILESDIR}/${PN}-2.6-revert_rootpath_messup.patch"
