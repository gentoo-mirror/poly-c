# Copyright 1999-2018 Gentoo Foundation
# Distributed under the terms of the GNU General Public License v2
# $Id: 43993d90bc5fedb6c621a65f6f07d3fa5b72425a $

EAPI=6

inherit flag-o-matic toolchain-funcs

DESCRIPTION="Tools and library to manipulate EFI variables"
HOMEPAGE="https://github.com/rhinstaller/efivar"
#SRC_URI="https://github.com/rhinstaller/efivar/releases/download/${PV}/${P}.tar.bz2"
SRC_URI="https://github.com/rhinstaller/efivar/archive/${PV}/${P}.tar.gz"

SRC_URI+=" https://github.com/rhboot/efivar/commit/cd732494ba7685feaf71b9ee58619ca6aef39fc9.patch -> ${PN}-32-efi_guid_ux_capsule.patch"

LICENSE="GPL-2"
SLOT="0/1"
KEYWORDS="~amd64 ~arm64 ~ia64 ~x86"

RDEPEND="dev-libs/popt"
DEPEND="${RDEPEND}
	>=sys-kernel/linux-headers-3.18
	virtual/pkgconfig
"

PATCHES=(
	"${DISTDIR}/${P}-efi_guid_ux_capsule.patch"
)

src_prepare() {
	default
	sed -i -e s/-Werror// gcc.specs || die
}

src_configure() {
	tc-export CC
	tc-ld-disable-gold
	export libdir="/usr/$(get_libdir)"
	unset LIBS # Bug 562004

	if [[ -n ${GCC_SPECS} ]]; then
		# The environment overrides the command line.
		GCC_SPECS+=":${S}/gcc.specs"
	fi
}
