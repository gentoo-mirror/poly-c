...
inherit rpm poly-c_ebuilds

BASE_PV=$(ver_cut 1-3)
REAL_PV="${MY_PV/_alpha/.alpha}"
REAL_PV="${REAL_PV/_beta/.beta}"
[[ ${MY_PV} == *alpha* || ${MY_PV} == *beta* ]] && PN_DEV="Dev"
...
for lang in ${LANGUAGES_HELP}; do
	helppack="offlinehelp? ( ${BASE_SRC_URI_STABLE}/x86/LibreOffice${PN_DEV}_${BASE_PV}_Linux_x86_rpm_helppack_${lang#*:}.tar.gz -> LibreOffice_${REAL_PV}_Linux_x86_rpm_helppack_${lang#*:}.tar.gz ${BASE_SRC_URI_TESTING}/x86/LibreOffice${PN_DEV}_${REAL_PV}_Linux_x86_rpm_helppack_${lang#*:}.tar.gz -> LibreOffice_${REAL_PV}_Linux_x86_rpm_helppack_${lang#*:}.tar.gz )"
	SRC_URI+=" l10n_${lang%:*}? ( ${helppack} )"
done
for lang in ${LANGUAGES}; do
	if [[ ${lang%:*} != en ]]; then
		langpack="${BASE_SRC_URI_STABLE}/x86/LibreOffice${PN_DEV}_${BASE_PV}_Linux_x86_rpm_langpack_${lang#*:}.tar.gz -> LibreOffice_${REAL_PV}_Linux_x86_rpm_langpack_${lang#*:}.tar.gz ${BASE_SRC_URI_TESTING}/x86/LibreOffice${PN_DEV}_${REAL_PV}_Linux_x86_rpm_langpack_${lang#*:}.tar.gz -> LibreOffice_${REAL_PV}_Linux_x86_rpm_langpack_${lang#*:}.tar.gz"
...
src_prepare() {
	...
	for lang in ${LANGUAGES}; do
		...
		if [[ ${lang%:*} != en ]]; then
			rpmdir="LibreOffice_${REAL_PV}_Linux_x86_rpm_langpack_${dir}/RPMS/"
			[[ -d ${rpmdir} ]] || die "Missing directory: ${rpmdir}"
			rpm_unpack ./${rpmdir}/*.rpm
		fi
		if [[ "${LANGUAGES_HELP}" =~ " ${lang} " ]] && use offlinehelp; then
			rpmdir="LibreOffice_${REAL_PV}_Linux_x86_rpm_helppack_${dir}/RPMS/"
			[[ -d ${rpmdir} ]] || die "Missing directory: ${rpmdir}"
			rpm_unpack ./${rpmdir}/*.rpm
		fi
	done
