...
inherit autotools eutils flag-o-matic gnome2-utils l10n multilib multilib-minimal pax-utils toolchain-funcs virtualx versionator xdg-utils poly-c_ebuilds
...
if [[ ${MY_PV} == "9999" ]] ; then
	...
	SRC_URI="https://dl.winehq.org/wine/source/${MAJOR_V}.x/${REAL_P}.tar.xz"
...
PATCHES=(
	"${PATCHDIR}/patches/${REAL_PN}-1.5.26-winegcc.patch" #260726
	"${PATCHDIR}/patches/${REAL_PN}-1.9.5-multilib-portage.patch" #395615
	"${PATCHDIR}/patches/${REAL_PN}-1.6-memset-O3.patch" #480508
	"${PATCHDIR}/patches/${REAL_PN}-2.0-multislot-apploader.patch"
)
PATCHES_BIN=()
...
wine_env_vcs_vars() {
	...
	if [[ ! -z ${pn_live_val} ]]; then
		if use staging || use d3d9; then
			eerror "Because of the multi-repo nature of ${REAL_PN}, ${pn_live_var}"
...
pkg_setup() {
	...
	REAL_PREFIX="${EPREFIX}/usr/lib/wine-${WINE_VARIANT}"
...
multilib_src_test() {
	...
	if [[ ${ABI} == x86 ]]; then
		if [[ $(id -u) == 0 ]]; then
			...
			ewarn "To run the test ${REAL_PN} suite, add userpriv to FEATURES in make.conf"
			return
		fi
...
multilib_src_install_all() {
	...
	if ! use perl ; then # winedump calls function_grep.pl, and winemaker is a perl script
		rm "${D%/}${REAL_PREFIX}"/bin/{wine{dump,maker},function_grep.pl} \
			"${D%/}${MY_MANDIR}"/man1/wine{dump,maker}.1 || die
	fi

	# Remove wineconsole if neither backend is installed #551124
	if ! use X && ! use ncurses; then
		rm "${D%/}${REAL_PREFIX}"/bin/wineconsole* || die
		rm "${D%/}${MY_MANDIR}"/man1/wineconsole* || die
		rm_wineconsole() {
			rm "${D%/}${REAL_PREFIX}/$(get_libdir)"/wine/{,fakedlls/}wineconsole.exe* || die
		}
		multilib_foreach_abi rm_wineconsole
	fi

	use abi_x86_32 && pax-mark psmr "${D%/}${REAL_PREFIX}"/bin/wine{,-preloader} #255055
	use abi_x86_64 && pax-mark psmr "${D%/}${REAL_PREFIX}"/bin/wine64{,-preloader}

	if use abi_x86_64 && ! use abi_x86_32; then
		dosym wine64 "${REAL_PREFIX}"/bin/wine # 404331
		dosym wine64-preloader "${REAL_PREFIX}"/bin/wine-preloader
	fi
	...
	local b
	for b in "${D%/}${REAL_PREFIX}"/bin/*; do
		make_wrapper "${b##*/}-${WINE_VARIANT}" "${REAL_PREFIX}/bin/${b##*/}"
	done
...
pkg_postinst() {
	eselect wine register ${MY_P}
	if [[ ${PN} == "wine-vanilla" ]]; then
		eselect wine register --vanilla ${MY_P} || die
	else
		if use staging; then
			eselect wine register --staging ${MY_P} || die
		fi
		if use d3d9; then
			eselect wine register --d3d9 ${MY_P} || die
		fi
	fi
...
pkg_prerm() {
	...
	if [[ ${PN} == "wine-vanilla" ]]; then
		eselect wine deregister --vanilla ${MY_P} || die
	else
		if use staging; then
			eselect wine deregister --staging ${MY_P} || die
		fi
		if use d3d9; then
			eselect wine deregister --d3d9 ${MY_P} || die
		fi
	fi
