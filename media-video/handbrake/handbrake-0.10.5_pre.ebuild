...
if [[ ${PV} = *9999* ]]; then
	...
	SRC_URI="http://handbrake.fr/rotation.php?file=HandBrake-${MY_PV}.tar.bz2 -> ${MY_P}.tar.bz2"
	S="${WORKDIR}/HandBrake-${MY_PV}"
	KEYWORDS="~amd64 ~x86"
fi
...
src_prepare() {
	...
	use libav || EPATCH_OPTS="-R" epatch "${FILESDIR}/${PN}-0.10.3-nolibav.patch"
