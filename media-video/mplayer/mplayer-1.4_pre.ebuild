...
[[ ${MY_PV} = *9999* ]] && SVN_ECLASS="subversion git-2" || SVN_ECLASS=""

inherit toolchain-funcs flag-o-matic ${SVN_ECLASS} poly-c_ebuilds
...
if [[ ${MY_PV} == *9999* ]]; then
	RELEASE_URI=""
elif [ "${MY_PV%_rc*}" = "${MY_PV}" -a "${MY_PV%_pre*}" = "${MY_PV}" ]; then
	REAL_P="MPlayer-${MY_PV}"
	S="${WORKDIR}/${REAL_P}"
	RELEASE_URI="mirror://mplayer/releases/${REAL_P}.tar.xz"
else
	RELEASE_URI="mirror://gentoo/${MY_P}.tar.xz"
fi
SRC_URI="${RELEASE_URI}
	!truetype? ( ${FONT_URI} )"
...
src_unpack() {
	if [[ ${MY_PV} = *9999* ]]; then
		...
		cd "${WORKDIR}"
		rm -rf "${WORKDIR}/${MY_P}/ffmpeg/"
		( S="${WORKDIR}/${MY_P}/ffmpeg/" git-2_src_unpack )
	else
		unpack ${A}
	fi

	if [[ ${MY_PV} = *9999* ]] || [[ "${MY_PV%_rc*}" = "${MY_PV}" ]]; then
		cd "${S}"
