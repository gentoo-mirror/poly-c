...
inherit autotools flag-o-matic lua-single toolchain-funcs virtualx xdg poly-c_ebuilds
...
if [[ ${MY_PV} = *9999 ]] ; then
	...
	SRC_URI="https://code.videolan.org/videolan/vlc-$(ver_cut 1-2)/-/archive/${MY_PV}/vlc-$(ver_cut 1-2)-${MY_PV}.tar.gz"
	S="${WORKDIR}/${PN}-$(ver_cut 1-2)-${MY_PV}"
	#if [[ ${REAL_P} = ${MY_P} ]] ; then
	#	SRC_URI="https://download.videolan.org/pub/videolan/${PN}/${MY_PV}/${MY_P}.tar.xz"
	#else
	#	SRC_URI="https://download.videolan.org/pub/videolan/testing/${REAL_P}/${REAL_P}.tar.xz"
	#fi
