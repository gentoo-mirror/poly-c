...
inherit pax-utils multilib poly-c_ebuilds
...
if [ "${MY_PV}" = "9999" ]; then
	...
	KEYWORDS="~amd64"
	SRC_URI="https://github.com/hashcat/hashcat/archive/v${MY_PV}.tar.gz -> ${MY_P}.tar.gz"
fi
...
src_prepare() {
	...
	export DOCUMENT_FOLDER="/usr/share/doc/${MY_P}"
	default
}

src_compile() {
	emake SHARED=1 PRODUCTION=1 ENABLE_BRAIN=$(usex brain 1 0) \
		USE_SYSTEM_LZMA=0 USE_SYSTEM_OPENCL=1 USE_SYSTEM_XXHASH=1 \
		USE_SYSTEM_ZLIB=1  VERSION_PURE="${MY_PV}"
	pax-mark -mr hashcat
}
...
src_test() {
	...
	LD_PRELOAD=./libhashcat.so.${MY_PV} ./hashcat -a 3 -m 1500 nQCk49SiErOgk || die "Test failed"
}

src_install() {
	emake DESTDIR="${ED}" SHARED=1 PRODUCTION=1 ENABLE_BRAIN=$(usex brain 1 0) USE_SYSTEM_LZMA=0 USE_SYSTEM_OPENCL=1 USE_SYSTEM_XXHASH=1 VERSION_PURE="${MY_PV}" install
}
