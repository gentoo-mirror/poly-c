...
inherit eutils systemd toolchain-funcs user poly-c_ebuilds
...
IUSE="+client gssapi ipv6 kernel_linux ldap selinux +server ssl vim-syntax"

REQUIRED_USE="gssapi? ( ldap )"
...
src_configure() {
	...
	econf \
		...
		$(use ldap && use_with gssapi ldap-gssapi || echo --without-ldap-gssapi)
	...
	eval econf \
		...
		--disable-symtable \
		$(use ldap && use_with gssapi || echo --without-gssapi) \
		--without-make-clean
}
