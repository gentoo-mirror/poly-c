...
REAL_PN=${PN}${MY_PV/\.*}
inherit desktop xdg-utils systemd poly-c_ebuilds

DESCRIPTION="All-In-One Solution for Remote Access and Support over the Internet"
HOMEPAGE="https://www.teamviewer.com"
SRC_URI="amd64? ( https://dl.tvcdn.de/download/linux/version_${MY_PV/\.*}x/${PN}_${MY_PV}_amd64.tar.xz )
		x86? ( https://dl.tvcdn.de/download/linux/version_${MY_PV/\.*}x/${PN}_${MY_PV}_i386.tar.xz )"
...
src_prepare() {
	...
	sed -e "/^ExecStart/s/${PN}/${REAL_PN}/" \
		-i tv_bin/script/teamviewerd.service || die
}

src_install() {
	local dst="/opt/${REAL_PN}" # install destination
	...
	# <major-version>')
	keepdir /var/log/${REAL_PN}
	dosym ../../var/log/${REAL_PN} ${dst}/logfiles
