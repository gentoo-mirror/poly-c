...
CDEPEND="
	...
	system-libvpx? ( >=media-libs/libvpx-1.8:0=[postproc] )
...
QA_PRESTRIPPED="usr/lib*/${PN}/waterfox-current"
...
src_prepare() {
	rm "${WORKDIR}"/firefox/2016_set_CARGO_PROFILE_RELEASE_LTO.patch \
		"${WORKDIR}"/firefox/3000_use_the_Mozilla_location_service_when_no_Google_key_is_available.patch \
		|| die
	...
	if use graceful-fingerprinting ; then
		eapply "${FILESDIR}"/graceful_fingerprinting.patch
	fi
...
src_configure() {
	...
	if use pgo ; then
		if ! has userpriv ${FEATURES} ; then
			eerror "Building waterfox-current with USE=pgo and FEATURES=-userpriv is not supported!"
		fi
	fi
	...
	mozconfig_annotate 'Waterfox' --with-branding=browser/branding/waterfox
...
src_install() {
	...
	# Add our default prefs for waterfox-current
	...
	icon_path="${S}/browser/branding/waterfox"
	icon="${PN}"
	name="Waterfox Current"

	# Disable built-in auto-update because we update waterfox-current through package manager
	...
	for display_protocol in ${display_protocols} ; do
		...
		case ${display_protocol} in
			Wayland)
				exec_command='waterfox-current-wayland --name waterfox-current-wayland'
				newbin "${FILESDIR}"/waterfox-current-wayland.sh waterfox-current-wayland
				;;
			X11)
				if ! use wayland ; then
				...
				exec_command='waterfox-current-x11 --name waterfox-current-x11'
				newbin "${FILESDIR}"/waterfox-current-x11.sh waterfox-current-x11
				;;
			*)
				app_name="${name}"
				desktop_filename="${PN}.desktop"
				exec_command='waterfox-current'
				;;
		esac

		newmenu "${FILESDIR}/icon/${PN}.desktop"  "${desktop_filename}"
		sed -i \
			-e "s:@NAME@:${app_name}:" \
	...
	rm "${ED%/}"/usr/bin/waterfox-current || die
	newbin "${FILESDIR}"/waterfox-current.sh waterfox-current

	local wrapper
	for wrapper in \
		"${ED%/}"/usr/bin/waterfox-current \
		"${ED%/}"/usr/bin/waterfox-current-x11 \
		"${ED%/}"/usr/bin/waterfox-current-wayland \
	; do
		[[ ! -f "${wrapper}" ]] && continue
	...
	# waterfox-current and waterfox-current-bin are identical
	# rm "${ED%/}"${MOZILLA_FIVE_HOME}/${PN}-bin || die
	# dosym waterfox-current ${MOZILLA_FIVE_HOME}/waterfox-current-bin

	# Required in order to use plugins and even run waterfox-current on hardened.
	pax-mark m "${ED}"${MOZILLA_FIVE_HOME}/{waterfox-current,plugin-container}
}
