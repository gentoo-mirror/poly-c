...
inherit eutils alternatives flag-o-matic toolchain-funcs multilib multiprocessing poly-c_ebuilds
...
REAL_P="perl-${DIST_VERSION}"
# Used in library paths
REAL_PV="${DIST_VERSION%-RC*}"

DESCRIPTION="Larry Wall's Practical Extraction and Report Language"

SRC_URI="
	mirror://cpan/src/5.0/${REAL_P}.tar.xz
	mirror://cpan/authors/id/${DIST_AUTHOR:0:1}/${DIST_AUTHOR:0:2}/${DIST_AUTHOR}/${REAL_P}.tar.xz
...
check_rebuild() {
	...
	elif [[ "${REPLACING_VERSIONS%.*}" != "${MY_PV%.*}" ]]; then
		echo ""
...
pkg_setup() {
	...
	LIBPERL="libperl$(get_libname ${REAL_PV} )"
	PRIV_LIB="${PRIV_BASE}/${REAL_PV}"
	ARCH_LIB="${PRIV_BASE}/${REAL_PV}/${myarch}${mythreading}"
	SITE_LIB="${SITE_BASE}/${REAL_PV}"
	SITE_ARCH="${SITE_BASE}/${REAL_PV}/${myarch}${mythreading}"
	VENDOR_LIB="${VENDOR_BASE}/${REAL_PV}"
	VENDOR_ARCH="${VENDOR_BASE}/${REAL_PV}/${myarch}${mythreading}"
...
src_remove_dual_man() {
	...
	case "${EBUILD_PHASE:-none}" in
		postinst|postrm)
			for i in "$@" ; do
				ff=`echo "${EROOT}${i%.[0-9]}-${ver}-${MY_P}${i#${i%.[0-9]}}"*`
				ff=${ff##*${i#${i%.[0-9]}}}
		...
		install)
			for i in "$@" ; do
				...
				mv "${ED}"${i} "${ED}"${i%.[0-9]}-${ver}-${MY_P}${i#${i%.[0-9]}} || die
			done
			;;
	esac
}
...
src_install() {
	...
	rm -f "${ED}/usr/bin/perl${REAL_PV}"
	ln -s perl "${ED}"/usr/bin/perl${REAL_PV} || die
