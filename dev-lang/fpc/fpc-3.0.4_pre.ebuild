...
inherit prefix toolchain-funcs poly-c_ebuilds

HOMEPAGE="https://www.freepascal.org/"
DESCRIPTION="Free Pascal Compiler"
SRC_URI="mirror://sourceforge/freepascal/fpcbuild-${MY_PV}.tar.gz
	mirror://sourceforge/freepascal/fpc-${MY_PV}.source.tar.gz
	amd64? ( mirror://sourceforge/freepascal/${MY_P}.x86_64-linux.tar )
	x86? ( mirror://sourceforge/freepascal/${MY_P}.i386-linux.tar )
	doc? ( mirror://sourceforge/freepascal/Documentation/${MY_PV}/doc-html.tar.gz -> ${MY_P}-doc-html.tar.gz )"
...
S=${WORKDIR}/fpcbuild-${MY_PV}/fpcsrc
...
src_unpack() {
	case ${ARCH} in
		amd64)	FPC_ARCH="x86_64"    PV_BIN=${MY_PV} ;;
		x86)	FPC_ARCH="i386"      PV_BIN=${MY_PV} ;;
...
src_prepare() {
	...
	sed -i '/find man.* gzip /d' "${WORKDIR}"/fpcbuild-${MY_PV}/install/man/Makefile || die

	# make the compiled binary check for fpc.cfg under the prefixed /etc/ path
	hprefixify "${WORKDIR}"/fpcbuild-${MY_PV}/fpcsrc/compiler/options.pas
}
...
src_install() {
	...
	#${WORKDIR}/${PN}build-${MY_PV}/utils/fpcm/fpcmake"
	...
	set -- PP="${pp}" FPCMAKE="${S}/utils/fpcm/bin/${FPC_PARCH}-linux/fpcmake" \
		...
		INSTALL_SOURCEDIR="${ED}"/usr/lib/fpc/${MY_PV}/source

	emake -j1 "$@" compiler_install rtl_install packages_install utils_install

	dosym ../lib/fpc/${MY_PV}/ppc${FPC_ARCH} /usr/bin/ppc${FPC_ARCH}
	...
	if use source ; then
		...
		find "${ED}"/usr/lib/fpc/${MY_PV}/source -name '*.o' -exec rm {} \;
	fi

	"${ED}"/usr/lib/fpc/${MY_PV}/samplecfg "${ED}"/usr/lib/fpc/${MY_PV} "${ED}"/etc || die
	...
	if use ide ; then
		sed -e "s:${ED}::g" \
			-i "${ED}"/etc/fppkg.cfg \
			-i "${ED}"/etc/fppkg/* \
			-i "${ED}"/usr/lib/fpc/${MY_PV}/ide/text/fp*.cfg \
			|| die
	fi
