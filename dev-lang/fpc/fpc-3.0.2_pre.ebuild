...
src_install() {
	...
	set -- PP="${pp}" FPCMAKE="${S}/utils/fpcm/bin/${FPC_PARCH}-linux/fpcmake" \
		...
		INSTALL_SOURCEDIR="${D}"/usr/lib/fpc/${MY_PV}/source
	...
	if use source ; then
		...
		find "${D}"/usr/lib/fpc/${MY_PV}/source -name '*.o' -exec rm {} \;
	fi

	"${D}"/usr/lib/fpc/${MY_PV}/samplecfg "${D}"/usr/lib/fpc/${MY_PV} "${D}"/etc || die
	...
	if use ide ; then
		sed -e "s:${D}::g" \
			-i "${D}"/etc/fppkg.cfg \
			-i "${D}"/etc/fppkg/* \
			-i "${D}"/usr/lib/fpc/${MY_PV}/ide/text/fp*.cfg \
			|| die
	fi
