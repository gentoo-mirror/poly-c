...
SRC_URI="https://www.python.org/ftp/python/${MY_PV}/${REAL_P}.tar.xz
...
src_prepare() {
	...
	local patch_exclude=(
		2.7.16/0010-tests-environment.patch
	)

	local mpatch
	for mpatch in ${patch_exclude[@]} ; do
		if [[ -f "${FILESDIR}/${mpatch}" ]] ; then
			cp "${FILESDIR}/${mpatch}" "${WORKDIR}/patches" || die
		else
			rm "${WORKDIR}/patches"/${mpatch} || die
		fi
	done
