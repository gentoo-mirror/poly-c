...
inherit autotools fdo-mime gnome2-utils flag-o-matic linux-info poly-c_ebuilds \
...
if [[ ${MY_PV} == *9999 ]]; then
	...
	if [[ ${MY_PV} != 9999 ]]; then
		EGIT_BRANCH=branch-${MY_PV/.9999}
	fi
else
	SRC_URI="https://github.com/apple/${PN}/archive/v${MY_PV}.tar.gz -> ${MY_P}.tar.gz"
		...
		if  ! linux_config_exists; then
			...
			if use usb; then
				if linux_chkconfig_present USB_PRINTER; then
					...
					eerror "${MY_P} requires the USB_PRINTER support disabled."
