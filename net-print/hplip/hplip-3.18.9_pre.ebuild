...
src_prepare() {
	if pushd "${WORKDIR}/patches" &>/dev/null ; then
		rm hplip-3.15.6-distribution.patch || die
		popd &>/dev/null || die
	fi

	PATCHES+=(
		"${FILESDIR}"/hplip-3.18.9-distribution.patch
		"${FILESDIR}"/hplip-3.18.9-destdir.patch
	)
