...
inherit autotools flag-o-matic linux-info python-single-r1 readme.gentoo-r1 udev poly-c_ebuilds
...
src_prepare() {
	rm "${WORKDIR}"/patches/005_hplip-3.18.10-remove-imageprocessor.patch \
		|| die
	rm "${WORKDIR}"/patches/017_hplip-3.20.9-thread-isalive-removed.patch \
		|| die
	cp ${FILESDIR}/005_hplip-3.21.4-remove-imageprocessor.patch \
		"${WORKDIR}"/patches/ || die
...
src_configure() {
	...
	append-flags -DDISABLE_IMAGEPROCESSOR
