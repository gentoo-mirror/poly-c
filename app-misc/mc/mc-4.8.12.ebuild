...
# $Id: 7074bfcd5334df2ee49696f7241950461a047c55 $
...
src_prepare() {
	epatch "${FILESDIR}"/${P}-dont_count_moves_twice.patch

	epatch "${FILESDIR}"/${PN}-4.8.9-fix-too-long-german-strings.patch \
		"${FILESDIR}"/${P}-po_update_fix.patch

	[[ -n ${LIVE_EBUILD} ]] && ./autogen.sh || eautoreconf
}
