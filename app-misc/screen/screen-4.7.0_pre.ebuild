...
inherit autotools eutils flag-o-matic pam tmpfiles toolchain-funcs user poly-c_ebuilds
...
if [[ "${MY_PV}" != 9999 ]] ; then
	...
	EGIT_CHECKOUT_DIR="${WORKDIR}/${MY_P}" # needed for setting S later on
	S="${WORKDIR}"/${MY_P}/src
fi
...
src_install() {
	...
	emake DESTDIR="${D}" SCREEN=screen-${MY_PV} install

	local tmpfiles_perms tmpfiles_group

	if use multiuser || use prefix
	then
		fperms 4755 /usr/bin/screen-${MY_PV}
		tmpfiles_perms="0755"
		tmpfiles_group="root"
	else
		fowners root:utmp /usr/bin/screen-${MY_PV}
		fperms 2755 /usr/bin/screen-${MY_PV}
		tmpfiles_perms="0775"
		tmpfiles_group="utmp"
	fi
